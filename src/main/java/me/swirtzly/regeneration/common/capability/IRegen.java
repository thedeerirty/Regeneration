...
public interface IRegen extends INBTSerializable<CompoundNBT> {
	...
	default boolean canRegenerate() {
        return (RegenConfig.COMMON.infiniteRegeneration.get() || getRegenerationsLeft() > 0) && getLivingEntity().getPosY() > 0 && !MinecraftForge.EVENT_BUS.post(new RegenerationEvent(getLivingEntity()));
	}
