...
public class TimelordGuardModel extends BipedModel<TimelordEntity> {
	...
	@Override
	public void setRotationAngles(TimelordEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch){
		IRegen regenData = RegenCap.get(entity).orElseGet(null);

		if (entity.getHeldItemMainhand().getItem() instanceof GunItem && regenData.getState() != PlayerUtil.RegenState.REGENERATING) {
			rightArmPose = BipedModel.ArmPose.BOW_AND_ARROW;
		}
		...
		RegenCap.get(entity).ifPresent((data) -> data.getType().create().getRenderer().postAnimation(this, entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch));
	...
	public void render(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha){
		timelord_hat.render(matrixStack, buffer, packedLight, packedOverlay);
